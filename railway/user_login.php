...
<body style=" background-image: url(station.jpg);
    ...
    $query = mysqli_query($conn, "SELECT * FROM user WHERE user.mobileno=$mobile AND user.password='" . $pwd . "' ");
    ...
    if ($row = mysqli_fetch_array($query)) {
        ...
        $query2 = mysqli_query($conn, " select * from user,resv where user.id=resv.id AND  user.mobileno=$mobile ");
